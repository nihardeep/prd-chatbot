...
export default function TravelApp() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Hi! I\'m your Trip Planner assistant. Tell me about your dream trip and I\'ll help you create the perfect itinerary!'
    }
  ]);
  ...
  const handleDestinationClick = (destination) => {
    ...
    setMessages([
      {
        role: 'assistant',
        content: `Great choice! Let's plan your trip to ${destination}. Tell me about your preferences, budget, and what activities interest you most?`
      }
    ]);
  };
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are an expert travel planner specializing in Southeast Asian destinations. Help users plan amazing trips to ${selectedDestination}. Provide specific recommendations for hotels, restaurants, attractions, activities, and create detailed itineraries. Be enthusiastic and engaging.`
            },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            {destinations.map((destination) => (
              ...
              >
                <div className="relative h-80 rounded-2xl overflow-hidden shadow-2xl transform transition hover:scale-105 hover:shadow-purple-500/50 duration-300">
                  ...
                  {/* Gradient Overlay */}
                  <div className={`absolute inset-0 bg-gradient-to-t ${destination.color} opacity-0 group-hover:opacity-40 transition duration-300`}></div>

                  {/* Content */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent flex flex-col justify-end p-6">
                    <h3 className="text-2xl font-bold text-white mb-1">{destination.name}</h3>
                    <p className="text-purple-200 mb-4">{destination.description}</p>
          ...
          {/* Trip Planner Widget - Bottom Right */}
